...
<%@ page import="app.modelos.Reserva, java.util.List, app.modelos.Cliente, app.modelos.Libro"%>
...
List<Cliente> listaClientes = (List<Cliente>) request.getAttribute("lsClientes");
%>
<%
@SuppressWarnings("unchecked")
List<Libro> listaLibros = (List<Libro>) request.getAttribute("lsLibros");
%>
...
<div class="container">
    ...
    <div class="d-flex justify-content-between mb-3">
        ...
        <a href="/SistemaBiblioteca/Inicio/Reserva/Buscar" class="btn btn-primary">Volver</a>
    </div>
    ...
    <form action="Editar" method="POST" name="fEditar">
     ...
     <input type="text" id="id" name="id" class="form-control mb-3" value="<%= reserva.getId() %>" readonly>
        ...
        <label for="cliente" class="form-label">Clientes:</label>
	        ...
	        <% for (Cliente c : listaClientes) {%>
	            <option  value="<%= c.getDni() %>"<%= c.getDni().equals(reserva.getCliente().getDni()) ? " selected" : ""%>><%= c.getNombre() %></option>
	        <% } %>
	        </select> 
	        ...
	        <% for (Libro l : listaLibros) {%>
	            <option  value="<%= l.getId() %>"<%= l.getId() == reserva.getLibro().getId() ? " selected" : ""%>><%= l.getTitulo() %></option>
	        <% } %>
	        </select>

        <label for="fecha" class="form-label">Fecha:</label>
        <input type="date" id="fecha" name="fecha" class="form-control mb-3" value="<%= reserva.getFecha() %>" required>
        
        <label for="fechaLimite" class="form-label">Fecha Limite:</label>
        <input type="date" id="fechaLimite" name="fechaLimite" class="form-control mb-3" value="<%= reserva.getFechaLimite() %>"  required>
